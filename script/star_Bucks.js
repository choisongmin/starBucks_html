          ...
          section2Slide: function(){
              ...
              var winW = $(window).innerWidth()*0.9;    //창너비의 90% 크기
                ...
                //    현재 슬라이드의 페이지버튼 자식요소인 이미지 src='경로/이미지' 속성변경
                  function pageEvent(){
                    // console.log( cnt );
                    $('.page-btn')        .children().attr('src','./img/main_prom_off.png')
                    $('.page-btn').eq(cnt==3?0:cnt).children().attr('src','./img/main_prom_on.png')
                ...
                //    클래스 활용 hasClass() 메서드 이용 : true  or  false 반환
                  ...
                  function stopFn(){
                    $('.play-btn').children().attr('src','./img/main_prom_play.png');
                    $('.play-btn').removeClass('on'); //삭제
                    // 슬라이드정지
                    clearInterval(setId);
                  }
                  function playFn(){
                    $('.play-btn').children().attr('src','./img/main_prom_stop.png');
